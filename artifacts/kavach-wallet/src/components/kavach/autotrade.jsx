...
export const AutoTradeTab = ({ interval = '1h' }) => {
  ...
  const { running, config, positions, history } = store;
  const [showConfig, setShowConfig] = useState(false);
  const [scanning, setScanning] = useState(false);
  const stats = useMemo(() => store.getStats(), [store.history, store.positions, store.equity]);
  ...
  useEffect(() => {
    ...
    const scan = async () => {
      ...
      try {
        ...
        store.processSignals(j.results, interval);
        // Build price map from scan results for open positions
        const priceMap = {};
        for (const r of j.results) priceMap[r.symbol] = r.entry;
        // Also fetch single signals for open position symbols not in scan
        for (const pos of store.positions) {
          if (priceMap[pos.symbol] == null) {
            try {
              const sr = await fetch(`/api/signal?symbol=${pos.symbol}&interval=${interval}`);
              const sj = await sr.json();
              if (sj.entry) priceMap[pos.symbol] = sj.entry;
            } catch {}
          }
        }
        store.updatePrices(priceMap);
      } catch (e) {
  ...
  }, [running, interval, config.scanIntervalSec, config.mode]);
  ...
  const handleToggle = () => {
    if (running) {
      ...
      store.start();
      toast.success(`🤖 Auto-trade bot started — ${config.mode === 'mayhem' ? 'MAYHEM' : 'Normal'} mode!`);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className={`border-0 bg-gradient-to-br p-4 ${running ? (config.mode === 'mayhem' ? 'from-red-950 via-orange-950 to-slate-950 ring-1 ring-red-500/40' : 'from-slate-900 via-slate-900 to-cyan-950 ring-1 ring-cyan-500/30') : 'from-slate-900 via-slate-900 to-slate-950'}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-slate-400">
                {running ? (scanning ? 'Scanning signals...' : `Monitoring • ${positions.length} open`) : 'Stopped'}
              </div>
            </div>
          </div>
          <button
        ...
        <div className="mt-3 flex rounded-xl border border-slate-700 bg-slate-950/60 p-1">
          <button
            ...
            className={`flex flex-1 items-center justify-center gap-1.5 rounded-lg py-2 text-xs font-medium transition ${
              config.mode === 'normal' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-1 items-center justify-center gap-1.5 rounded-lg py-2 text-xs font-medium transition ${
              config.mode === 'mayhem' ? 'bg-red-900/60 text-red-300' : 'text-slate-400 hover:text-white'
            }`}
          >
        ...
        {config.mode === 'mayhem' && (
          <div className="mt-2 flex items-center gap-1.5 rounded-lg bg-red-500/10 px-3 py-1.5 text-[10px] text-red-300">
            ...
            <span>Mayhem: Grade B++, max {config.maxPositions} pos, {config.riskPerTradePct}% risk/trade, {config.scanIntervalSec}s scan</span>
          </div>
        )}
      </Card>
      ...
      <button
        ...
        className="flex w-full items-center justify-between rounded-xl border border-slate-800 bg-slate-900/60 px-3 py-2.5 text-left"
      >
      ...
      <div>
        ...
        {positions.length === 0 ? (
          <Card className="border-slate-800 bg-slate-900/40 p-6 text-center text-xs text-slate-500">
            {running ? 'Waiting for signals...' : 'Start the bot to begin trading.'}
          </Card>
        ) : (
          <div className="space-y-2">
            <AnimatePresence>
              {positions.map((p) => (
                <PositionCard key={p.id} pos={p} onClose={() => store.closePosition(p.id)} />
              ))}
            </AnimatePresence>
          </div>
        )}
      ...
      {history.length > 0 && (
        <div>
          <div className="mb-2 flex items-center justify-between">
            ...
            <button onClick={() => { if (window.confirm('Reset all trades and equity?')) store.reset(); }} className="text-[10px] text-slate-500 hover:text-red-400">Reset</button>
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5">
              ...
              <Badge className={`h-4 px-1.5 py-0 text-[9px] ${GRADE_COLOR[pos.grade]}`}>{pos.grade}</Badge>
            ...
            <div className="mt-0.5 flex items-center gap-2 text-[10px] text-slate-500">
              ...
              {pos.tpHit > 0 && <span className="text-emerald-400">· TP{pos.tpHit} hit</span>}
            </div>
          </div>
        ...
        <div className="mt-2 flex items-center gap-2">
          <div className="flex-1 grid grid-cols-3 gap-1 text-center">
            <div className="rounded bg-slate-800/40 px-1 py-0.5">
              <div className="text-[8px] text-slate-500">SL</div>
              <div className="font-mono text-[9px] text-red-300">${fmtPrice(pos.trailingStop || pos.stopLoss)}</div>
            </div>
            <div className="rounded bg-slate-800/40 px-1 py-0.5">
              <div className="text-[8px] text-slate-500">TP Next</div>
              <div className="font-mono text-[9px] text-emerald-300">${fmtPrice(pos.takeProfits?.[pos.tpHit])}</div>
            </div>
            <div className="rounded bg-slate-800/40 px-1 py-0.5">
              <div className="text-[8px] text-slate-500">Size</div>
              <div className="font-mono text-[9px] text-slate-300">{pos.size.toFixed(4)}</div>
            </div>
          </div>
    ...
    <div className="flex items-center gap-2 rounded-lg border border-slate-800/60 bg-slate-900/40 px-3 py-2">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          ...
          <Badge className={`h-3.5 px-1 py-0 text-[8px] ${GRADE_COLOR[trade.grade]}`}>{trade.grade}</Badge>
        </div>
        <div className="text-[10px] text-slate-500">{trade.reason} · {trade.duration ? `${(trade.duration / 1000 / 60).toFixed(0)}m` : ''}</div>
      </div>
      <div className="text-right">
        <div className={`font-mono text-xs font-bold ${isWin ? 'text-emerald-400' : 'text-red-400'}`}>
          {isWin ? '+' : ''}{fmtUsd(trade.pnl)}
        </div>
        <div className={`text-[9px] ${isWin ? 'text-emerald-400' : 'text-red-400'}`}>
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-1.5">
              {grades.map((g) => (
                <button
                  key={g}
                  onClick={() => onUpdate({ minGrade: g })}
                  className={`flex-1 rounded-lg py-2 text-xs font-bold transition ${
                    config.minGrade === g ? 'bg-cyan-500/20 text-cyan-300 ring-1 ring-cyan-500/40' : 'bg-slate-900 text-slate-400'
                  }`}
                >
                  {g}
                </button>
          ...
          <div className="flex items-center justify-between rounded-xl border border-slate-800 bg-slate-900/60 px-3 py-2.5">
            ...
            <button
              onClick={() => onUpdate({ trailingStop: !config.trailingStop })}
          ...
          <div className="flex items-center justify-between rounded-xl border border-slate-800 bg-slate-900/60 px-3 py-2.5">
            ...
            <button
              onClick={() => onUpdate({ autoCompound: !config.autoCompound })}
